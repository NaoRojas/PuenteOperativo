...
void *El_Duende_del_Puente(void *arg);
...
int main(int arg, char const *argv[])
{
    ...
    {
        printf("-------------------------------------------------------------- \n");
        printf("--------------------MODO Oficial de Tránsito------------------ \n");
        printf("-------------------------------------------------------------- \n");
        s = 0;
    };
    break;
    ...
    else
    {
        ...
        pthread_attr_t attr2;
        pthread_attr_init(&attr2);
        pthread_create(&tid2, &attr2, &El_Duende_del_Puente, NULL);
...
void *Puente_A_Terabithia(void *arg)
{
    ...
    while (1)
    {
        ...
        if (aux2)
        {
            ...
            if (aux2->direccion)
            {
                printf("Esta cruzando desde el Este: %s, velocidad: %2.f\n",
                       aux2->nombre, aux2->velocidad);
            }
            else
            {
                printf("Esta cruzando desde el Oeste: %s, velocidad: %2.f\n",
                       aux2->nombre, aux2->velocidad);
            }
...
void *El_Duende_del_Puente(void *arg)
{
    ...
    while (1)
    {
        ...
        if (d)
        {
            contadorE++;
            aux2 = deQueue(colitaE);
            if (contadorE == ESTE.k_veh_x_pas)
            {
                printf("El policia cambia de sentido el puente: Oeste -> Este\n");
                PUENTE.direccion = 0;
                contadorE = 0;
            }
        }
        else
        {
            contadorO++;
            aux2 = deQueue(colitaO);
            if (contadorO == OESTE.k_veh_x_pas)
            {
                printf("El policia cambia de sentido el puente: Este -> Oeste\n");
                PUENTE.direccion = 1;
                contadorO = 0;
            }
        }
